...
class IntegratedSwarmSystem:
            ...
            for j in range(self.num_drones):
                ...
                if dist < self.avoidance_radius and dist > 0.001:
                    ...
                    if dist < self.collision_radius:
                        ...
                        strength = (self.avoidance_radius - dist) / (self.avoidance_radius - self.collision_radius)
    ...
    def _compute_control(self, dt: float) -> np.ndarray:
        ...
        # 3. Blend
        command = (
            self.avoidance_strength * separation +
            tracking
        )
        ...
        for i in range(self.num_drones):
            ...
            # Z limit
            command[i, 2] = np.clip(command[i, 2], -self.max_velocity_z, self.max_velocity_z)
    ...
    def _control_loop(self):
        ...
        while self.running:
            ...
            self.estimated_positions, self.estimated_velocities = self.sensors.update(actual_dt)
