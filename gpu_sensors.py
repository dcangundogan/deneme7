...
if os.path.exists(_MATLAB_LIB_PATH):
    ...
    # Also set for ctypes to find the library
    import ctypes
    try:
...
try:
    ...
    import numpy as cp
    print(f"[GPU_SENSORS] GPU kullanılamıyor, CPU kullanılıyor: {type(e).__name__}")
...
class GPUSensorSimulator:
    ...
    def get_fused_state(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if GPU_AVAILABLE:
            return cp.asnumpy(self.fused_positions), cp.asnumpy(self.fused_velocities)
    ...
    def get_raw_sensors(self) -> dict:
        ...
        if GPU_AVAILABLE:
            return {
                ...
                'imu_gyro': cp.asnumpy(self.imu_gyro),
                'gps_position': cp.asnumpy(self.gps_position),
                'gps_velocity': cp.asnumpy(self.gps_velocity),
                'gps_valid': cp.asnumpy(self.gps_valid),
                'baro_altitude': cp.asnumpy(self.baro_altitude),
                'mag_heading': cp.asnumpy(self.mag_heading),
            }
        return {
    ...
    def get_estimation_error(self) -> dict:
        ...
        if GPU_AVAILABLE:
            pos_error = cp.asnumpy(pos_error)
            vel_error = cp.asnumpy(vel_error)
